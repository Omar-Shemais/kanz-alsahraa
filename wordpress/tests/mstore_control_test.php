...
try {
    ...
    $record = kanz_v3_mstore_record(); check(!is_wp_error($record), 'automatic MStore read');
    $valid->Setting->MainColor = '#abcdef';
    check(kanz_v3_mstore_publish($valid, $record['revision']) === true, 'atomic publication');
    check(json_decode(file_get_contents($path))->Setting->MainColor === '#abcdef', 'same file updated');
    check(is_wp_error(kanz_v3_mstore_publish($valid, $record['revision'])), 'stale editor rejected');
    check(kanz_v3_mstore_record()['revision'] !== $record['revision'], 'new revision loaded without import');
    $sealed = kanz_v3_fcm_seal('TEST PRIVATE MATERIAL');
    check(is_string($sealed) && strpos($sealed, 'TEST PRIVATE MATERIAL') === false, 'no plaintext secret');
    check(kanz_v3_fcm_unseal($sealed) === 'TEST PRIVATE MATERIAL', 'encrypted round trip');
    $bytes = base64_decode($sealed); $bytes[28] = chr(ord($bytes[28]) ^ 1);
    check(is_wp_error(kanz_v3_fcm_unseal(base64_encode($bytes))), 'ciphertext tampering rejected');
    $GLOBALS['test_token'] = str_repeat('A', 120);
    $payload = kanz_v3_notification_payload('Test', 'Test');
    $test = kanz_v3_notification_audience($payload, 'test_user', '7');
    check(isset($test['message']['token']) && !isset($test['message']['topic']), 'single device only');
    $GLOBALS['test_token'] = '';
    check(is_wp_error(kanz_v3_notification_audience($payload, 'test_user', '7')), 'missing token never broadcasts');
    check(is_wp_error(kanz_v3_notification_audience($payload, 'test_user', '999')), 'unknown account rejected');
    check(is_wp_error(kanz_v3_notification_audience($payload, 'other', '7')), 'unknown audience rejected');
    check(isset(kanz_v3_notification_audience($payload, 'broadcast')['message']['topic']), 'explicit marketing broadcast');
