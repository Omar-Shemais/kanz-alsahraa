...
if (!function_exists('kanz_v6_config_validate')) {
function kanz_v6_snippet_admin_js() {
...
function kanz_v6_destination_data($type, $value = '') {
...
function kanz_v6_destination_options($kind) {
    if ($kind === 'category') {
...
function kanz_v6_notification_payload($title, $body, $destination = array()) {
    ...
    if ($destination) {
        $checked = kanz_v6_destination_data(isset($destination['kanz_target']) ? $destination['kanz_target'] : '', isset($destination['kanz_id']) ? $destination['kanz_id'] : (isset($destination['kanz_url']) ? $destination['kanz_url'] : ''));
...
function kanz_v6_notification_credentials() {
    ...
    if (!$account) {
        ...
        if (is_string($stored) && trim($stored) !== '') {
            $decoded = kanz_v6_fcm_unseal($stored);
...
function kanz_v6_fcm_unseal($stored) {
...
function kanz_v6_notification_audience($payload, $audience, $user_id = '') {
...
function kanz_v6_notification_send($payload) {
    $account = kanz_v6_notification_credentials();
...
add_action('admin_post_kanz_send_notification', function () {
    ...
    $destination = kanz_v6_destination_data($type, $value);
    if (is_wp_error($destination)) { wp_die(esc_html($destination->get_error_message())); }
    $payload = kanz_v6_notification_payload($title, $body, $destination);
    ...
    $payload = kanz_v6_notification_audience($payload, $audience, $test_user);
    if (is_wp_error($payload)) { wp_die(esc_html($payload->get_error_message())); }
    $credentials = kanz_v6_notification_credentials();
    ...
    $result = kanz_v6_notification_send($payload);
...
add_action('admin_post_kanz_save_fcm_key', function () {
    ...
    $sealed = kanz_v6_fcm_seal(wp_json_encode($data));
...
function kanz_v6_notification_page_section() {
    $credentials = kanz_v6_notification_credentials();
    ...
    <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post">
      ...
      <p id="kanz-notif-row-category" style="display:none"><label>التصنيف
        ...
        <?php foreach (kanz_v6_destination_options('category') as $option) {
      ...
      <p id="kanz-notif-row-product" style="display:none"><label>المنتج
        ...
        <?php foreach (kanz_v6_destination_options('product') as $option) { ?><option value="<?php echo esc_attr($option['id']); ?>"><?php echo esc_html($option['name'] . ' — #' . $option['id']); ?></option><?php } ?>
        </select></label></p>
...
function kanz_v6_config_validate($data) {
...
add_action('admin_menu', function () {
    add_menu_page('إدارة تطبيق كنز', 'تطبيق كنز', 'manage_options', 'kanz-app-control', 'kanz_v6_config_page', 'dashicons-smartphone');
});
...
add_action('admin_enqueue_scripts', function ($hook) {
    ...
    wp_localize_script('jquery-core', 'kanzAdmin', array('categories' => $categories, 'products' => kanz_v6_destination_options('product')));
    wp_add_inline_script('jquery-core', kanz_v6_snippet_admin_js(), 'after');
});

function kanz_v6_mstore_path() {
...
function kanz_v6_mstore_record() {
    $path = kanz_v6_mstore_path();
    ...
    if (!is_object($object) || kanz_v6_config_validate($data) !== true) { return new WP_Error('mstore_invalid', 'ملف MStore الحالي غير صالح؛ لم يتم استبداله.'); }
    return array('revision' => hash('sha256', $raw), 'saved_at' => gmdate('c', filemtime($path)), 'config' => $object);
}

function kanz_v6_mstore_publish($object, $expected_revision) {
    $path = kanz_v6_mstore_path();
    if (is_wp_error($path)) { return $path; }
    ...
    try {
        ...
        if (function_exists('do_action')) { do_action('litespeed_purge_url', kanz_v6_mstore_url()); }
        return true;
    } finally { if (is_file($temporary)) { unlink($temporary); } }
}

function kanz_v6_mstore_url() {
...
add_action('admin_post_kanz_save_config', function () {
    ...
    $valid = kanz_v6_config_validate($data);
    ...
    $saved = kanz_v6_mstore_publish($object, (string) $revision);
...
/**
 * Validate the authenticated cookie returned by the MStore/Digits login API.
 * The password endpoint never accepts an email, phone number or user ID as
 * proof of identity; callers must already own a valid WordPress auth cookie.
 */
function kanz_v6_password_cookie_identity($request) {
    $cookie = trim((string) $request->get_header('User-Cookie'));
    if ($cookie === '' || strlen($cookie) > 4096) {
        return new WP_Error('kanz_password_unauthorized', 'Authentication is required.', array('status' => 401));
    }
    $candidates = array($cookie);
    $decoded = rawurldecode($cookie);
    if ($decoded !== $cookie) { $candidates[] = $decoded; }
    foreach ($candidates as $candidate) {
        foreach (array('logged_in', 'auth', 'secure_auth') as $scheme) {
            $user_id = wp_validate_auth_cookie($candidate, $scheme);
            if ($user_id) {
                return array('user_id' => (int) $user_id, 'scheme' => $scheme);
            }
        }
    }
    return new WP_Error('kanz_password_unauthorized', 'The login session is invalid or expired.', array('status' => 401));
}

function kanz_v6_password_permission($request) {
    $identity = kanz_v6_password_cookie_identity($request);
    if (is_wp_error($identity)) { return $identity; }
    wp_set_current_user($identity['user_id']);
    return true;
}

function kanz_v6_set_account_password($request) {
    $identity = kanz_v6_password_cookie_identity($request);
    if (is_wp_error($identity)) { return $identity; }

    // Rate limit per authenticated account and direct peer IP. Do not trust
    // forwarded headers here because they can be supplied by the caller.
    $peer = isset($_SERVER['REMOTE_ADDR']) ? (string) $_SERVER['REMOTE_ADDR'] : 'unknown';
    $rate_key = 'kanz_pwd_' . substr(hash('sha256', $identity['user_id'] . '|' . $peer), 0, 32);
    $attempts = (int) get_transient($rate_key);
    if ($attempts >= 5) {
        return new WP_Error('kanz_password_rate_limited', 'Too many attempts. Try again later.', array('status' => 429));
    }
    set_transient($rate_key, $attempts + 1, 10 * MINUTE_IN_SECONDS);

    $password = $request->get_param('password');
    $confirmation = $request->get_param('password_confirmation');
    if (!is_string($password) || !is_string($confirmation) || !hash_equals($password, $confirmation)) {
        return new WP_Error('kanz_password_mismatch', 'Password confirmation does not match.', array('status' => 400));
    }
    $length = function_exists('mb_strlen') ? mb_strlen($password, 'UTF-8') : strlen($password);
    if ($length < 8 || $length > 128) {
        return new WP_Error('kanz_password_invalid', 'Password must contain between 8 and 128 characters.', array('status' => 400));
    }

    $user_id = (int) $identity['user_id'];
    if (!get_userdata($user_id)) {
        return new WP_Error('kanz_password_user_missing', 'Account is unavailable.', array('status' => 404));
    }

    // wp_set_password invalidates existing sessions. Return a freshly signed
    // cookie so the newly registered app session remains authenticated.
    wp_set_password($password, $user_id);
    $expiration = time() + (14 * DAY_IN_SECONDS);
    $new_cookie = wp_generate_auth_cookie($user_id, $expiration, 'logged_in');
    if (!is_string($new_cookie) || $new_cookie === '') {
        return new WP_Error('kanz_password_cookie_failed', 'Password was updated; sign in again.', array('status' => 500));
    }
    delete_transient($rate_key);

    $response = new WP_REST_Response(array('success' => true, 'cookie' => $new_cookie), 200);
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('kanz/v1', '/account/password', array(
        'methods' => 'POST',
        'permission_callback' => 'kanz_v6_password_permission',
        'callback' => 'kanz_v6_set_account_password',
    ));
});

function kanz_v6_config_page() {
    ...
    <div class="wrap" dir="rtl">
      ...
      <p>رابط إعدادات MStore: <code dir="ltr"><?php echo esc_html(kanz_v6_mstore_url()); ?></code></p>
      ...
      <?php } ?>
      <?php kanz_v6_notification_page_section(); ?>
    </div>
    <?php
}
}
